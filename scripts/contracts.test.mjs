...
test('marketplace hardening exposes location, seller stores, chat, and optimistic favorites', async () => {
  ...
  const chatRoomsApi = await read('src/app/api/chat/rooms/route.ts')
  ...
  const realtimeTypingMigration = await read('supabase/migrations/20260926003000_chat_realtime_typing_authorization.sql')
  ...
  assert.match(chatUi, /useRouter/)
  assert.match(chatUi, /RealtimeChannel/)
  assert.match(chatUi, /deba-chat:/)
  assert.match(chatUi, /config: \{ private: true \}/)
  assert.match(chatUi, /onTyping/)
  assert.match(chatUi, /router\.push/)
  assert.match(chatUi, /onNavigate/)
  assert.doesNotMatch(chatUi, /window\.location\.(href|assign|replace)/)
  assert.match(chatUi, /src="\/deba-comms\.html"/)
  assert.match(chatSurface, /DEBA COMMS/)
  assert.match(chatSurface, /accountUsername/)
  assert.match(chatSurface, /نشط الآن/)
  assert.match(chatSurface, /setProductState/)
  assert.match(chatSurface, /productContextLoading/)
  assert.match(chatSurface, /productThumbPlaceholder/)
  assert.match(chatSurface, /id="attachmentInput"/)
  assert.match(chatSurface, /data-testid="chat-info"/)
  assert.match(chatSurface, /data-testid="chat-more"/)
  assert.match(chatSurface, /data-testid="chat-call"/)
  assert.match(chatSurface, /data-action="pin"/)
  assert.match(chatSurface, /data-action="reply"/)
  assert.match(chatSurface, /data-action="forward"/)
  assert.match(chatSurface, /data-action="copy"/)
  assert.match(chatSurface, /data-action="report"/)
  assert.match(chatSurface, /data-action="delete"/)
  assert.match(chatSurface, /function pinContextMessage/)
  assert.match(chatSurface, /function replyToContextMessage/)
  assert.match(chatSurface, /function copyContextMessage/)
  assert.match(chatSurface, /function reportContextMessage/)
  assert.match(chatSurface, /function deleteContextMessage/)
  ...
  assert.match(chatSurface, /product-message-media/)
  assert.match(chatSurface, /activeProductMessage\?\.productId/)
  assert.match(chatSurface, /area\.appendChild\(row\)/)
  assert.doesNotMatch(chatSurface, /product-starter-card/)  assert.match(chatSurface, /conversation-empty-state/)
  ...
  assert.match(chatSurface, /offer-btn decline/)
  assert.match(chatSurface, /data-testid="chat-attach-file"/)
  assert.match(chatSurface, /data-testid="chat-attach-image"/)
  assert.match(chatSurface, /data-testid="chat-share-location"/)
  assert.match(chatSurface, /function showHelpDialog/)
  assert.match(chatSurface, /function showAboutDialog/)
  assert.match(chatSurface, /activeProductMessage/)
  assert.match(chatSurface, /عرض الإعلان/)
  assert.match(chatSurface, /offer-btn accept/)
  assert.match(chatSurface, /offer-btn counter/)
  assert.match(chatSurface, /offer-btn decline/)
  assert.match(chatSurface, /messagesLoading/)
  assert.match(chatSurface, /setTypingState/)
  assert.match(chatSurface, /otherParticipantTyping/)
  assert.match(chatSurface, /typing\.hidden = !otherParticipantTyping/)
  assert.match(chatSurface, /onTyping\(hasText\)/)
  assert.doesNotMatch(chatSurface, /typing\.hidden = false/)
  assert.match(chatSurface, /function navigate\(destination, event\)/)
  assert.match(chatSurface, /navigate\('home', event\)/)
  assert.match(chatSurface, /navigate\('profile', event\)/)
  assert.match(chatSurface, /navigate\('product', event\)/)
  assert.doesNotMatch(chatSurface, /window\.history\.back/)
  assert.match(chatSurface, /const chats = \[\]/)
  assert.doesNotMatch(chatSurface, /const chats = \[\s*\n\s*\{/)
  assert.doesNotMatch(chatSurface, /songtone1/)
  assert.doesNotMatch(chatSurface, /شليور لاسلكي 18V/)
  assert.doesNotMatch(chatSurface, /images\.unsplash\.com/)
  assert.match(chatSurface, /data-state="loading"/)
  ...
  assert.match(chatRoomsApi, /chat product context load failed/)
  assert.match(chatRoomsApi, /from\('categories'\)/)
  ...
  assert.match(realtimeTypingMigration, /on realtime\.messages/)
  assert.match(realtimeTypingMigration, /extension = 'broadcast'/)
  assert.match(realtimeTypingMigration, /private\.is_chat_participant/)
  assert.match(realtimeTypingMigration, /deba-chat:/)
