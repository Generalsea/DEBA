...
export async function POST(request:Request){
  try{
    ...
    if(groups.some((g)=>!isUuid(g.seller_id)||!['pickup','seller_delivery','platform_delivery'].includes(g.delivery_method)||g.items.length<1||g.items.some((i)=>!isUuid(i.product_id)||i.quantity<1||i.quantity>100)))return NextResponse.json({error:'بيانات السلة غير مكتملة.'},{status:400})
