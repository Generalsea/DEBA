-- Keep fraud/risk signals internal. Buyers only receive a generic review-needed response.
...
drop policy if exists risk_assessments_admin_select on public.risk_assessments;

create policy risk_assessments_admin_select
on public.risk_assessments
for select to authenticated
using ((select private.is_admin()));

revoke execute on function public.assess_order_risk(uuid) from public, anon, authenticated;
...
begin
  ...
  return jsonb_build_object(
    'order_id',v_order.id,'reference_code',v_order.reference_code,
    'status',v_order.status,'quantity',p_quantity,
    'total',v_order.total,'currency',v_order.currency,'existing',false
  );
end;
$$;
