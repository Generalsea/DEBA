-- Phase 2 / Step 2.1
-- Handles transliterated vehicle-model intent and adds a guarded brand-only
-- fallback when a multi-token brand query has no strict match.
        ...
        or not exists (
          ...
          where not exists (
            ...
            where bs.group_key = requested_brand.group_key
              and position(' ' || bs.normalized_term || ' ' in ' ' || p.search_text || ' ') > 0
          )
        )
      )
  ),
        ...
        case
          ...
          when exists (
            ...
            where bs.group_key = any(brand_groups)
              and position(' ' || bs.normalized_term || ' ' in ' ' || filtered.normalized_title || ' ') > 0
          ) then 3.0
          else 1.0
        end
        +
