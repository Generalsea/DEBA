...
begin
  ...
  set
    ...
    quantity = p_quantity,
    city = case
      when p_city is null then v_product.city
      else nullif(btrim(p_city), '')
    end,
    governorate = case
      when p_governorate is null then v_product.governorate
      else nullif(btrim(p_governorate), '')
    end,
    district = case
      when p_district is null then v_product.district
      else nullif(btrim(p_district), '')
    end,
    updated_at = now(),
    status = 'draft',
